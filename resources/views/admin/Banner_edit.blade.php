...
<form class="layui-form" action="" method="post" style="margin: 20px 10px;">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="ba_desc"  value="{{$res->ba_desc}}" lay-verify="required" lay-reqtext="描述必填" placeholder="请输入简介" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item" style="height: 120px">
        ...
        <div style="display: flex;margin-left:100px;position: relative;top: -30px;">
            <div class="layui-upload">
                ...
                <input type="text" hidden id="n_icon"  value="{{$res->ba_urls}}" lay-verify="required" lay-reqtext="缩略图必须上传" name="ba_url">
            </div>
            <div class="layui-upload-list" style="margin: 0 20px">
                <img class="layui-upload-img" src="{{$res->ba_urls}}" style="width: 60px;height: 60px" id="demo1">
                <p id="demoText"></p>
            </div>
        </div>
    </div>
...
<script>
    layui.use( ['upload','form'], function() {
        ...
        form.on('submit(demo1)', function(data){
            $.post('/my_admin/banner/{{$res->ba_id}}',{_token: '{!! csrf_token() !!}',data:data.field,_method:'put'},function(res){
                if(res.code===0){
